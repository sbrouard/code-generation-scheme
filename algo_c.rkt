...
; + print in file the ligne of affectation, ex : int x5 = x3 * x2;
  ...
  (cond [(or (null? variable-list) (void? variable-list)) 
         ...
         (let* ([variable-list2 (cond [(and (not (is-reused-variable? (caddr list3) (cdr alist)))
                                      ...
                                      [(and (not (is-reused-variable? (cadr list3) (cdr alist)))
                                            (or (is-reused-variable? (caddr list3) (cdr alist)) (equal? 'x (caddr list3)))
                                            (not (equal? (caddr list3) (cadr list3))) (not (equal? 'x (cadr list3))) (not (number? (cadr list3))))
...
;alist = addlist in the begining
; variable-list : list whaere we stock all variables which have been used, '() for the first apeal
; apeal affectation-in for each elt (affectation) of alist
...
; write the complete c code in file
(define (complete-file-in file l)
  ...
  (cond [(null? l) (raise "ERROR : l musn't be empty")]
        ...
        [else (all-affectation-in (cons '(1 x 1) (cdr (addlist2list-of-list l l '()))) '() file)])
  (fprintf file "}")
  )
...
; n = power
; op = *
; in = 1 -> inplace; in = 0 -> outplace
(define (global-func n op in)
  (cond [(not (or (= 1 in) (= 0 in))) (raise "ERROR : in = 1 (inplace) or 0 (outplace)")]
        [(= 1 in)
         (define file (open-output-file "test.c"))
         (complete-file-in file (gen-min-string-eucl n))
         (close-output-port file)]
        [(= 0 in)
         (define file (open-output-file "test.c"))
         (complete-file-out file (gen-min-string-eucl n))
         (close-output-port file)]))
